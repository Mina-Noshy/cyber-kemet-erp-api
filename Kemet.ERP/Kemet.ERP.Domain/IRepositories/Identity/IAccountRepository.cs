...
namespace Kemet.ERP.Domain.IRepositories.Identity
{
    public interface IAccountRepository
    {
        ...
        Task<AppUser?> ConfirmEmailAsync(string userId, string token, CancellationToken cancellationToken = default);
        Task<AppUser?> SendConfirmationEmailAsync(string email, CancellationToken cancellationToken = default);
    }
}
