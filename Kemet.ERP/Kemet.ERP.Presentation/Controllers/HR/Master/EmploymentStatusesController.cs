...
{
    [Authorize]
    public class EmploymentStatusesController : BaseHrController
    {
        private readonly IEmploymentStatusMasterService _service;
        public EmploymentStatusesController(IEmploymentStatusMasterService service)
            => _service = service;
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync(EmploymentStatusMasterDto request, CancellationToken cancellationToken)
            => FormatHttpResponse(await _service.CreateAsync(request, cancellationToken));

        [HttpPut("{id:long}")]
        public async Task<IActionResult> UpdateAsync(long id, EmploymentStatusMasterDto request, CancellationToken cancellationToken)
